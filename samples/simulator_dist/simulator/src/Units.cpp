...
void exportUnits(py::module &m)
{
    ...
    m.def("ft2m",py::vectorize(ft2m));
    m.def("m2ft",py::vectorize(m2ft));
    m.def("deg2rad",py::vectorize(deg2rad));
    m.def("rad2deg",py::vectorize(rad2deg));
    m.def("N2kgf",py::vectorize(N2kgf));
    m.def("kgf2N",py::vectorize(kgf2N));
    m.def("lb2kg",py::vectorize(lb2kg));
    m.def("kg2lb",py::vectorize(kg2lb));
    m.def("N2lbf",py::vectorize(N2lbf));
    m.def("lbf2N",py::vectorize(lbf2N));
    m.def("psi2Pa",py::vectorize(psi2Pa));
    m.def("Pa2psi",py::vectorize(Pa2psi));
    m.def("slug2N",py::vectorize(slug2N));
    m.def("N2slug",py::vectorize(N2slug));
    m.def("slugft22kgm2",py::vectorize(slugft22kgm2));
    m.def("atmosphere",&atmosphere);
    m.def("horizon",py::vectorize(horizon));
}
