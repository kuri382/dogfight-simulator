...
void SixDoFFighter::initialize(){
    ...
    motion.setOmega(getValueFromJsonKRD(instanceConfig,"omega",randomGen,Eigen::Vector3d(0,0,0)));

}

void SixDoFFighter::makeChildren(){
    BaseType::makeChildren();
    if(engine.expired()){
        throw std::runtime_error("SixDoFFighter requires an engine as its component.");
    }
}

void SixDoFFighter::serializeInternalState(asrc::core::util::AvailableArchiveTypes & archive, bool full){
...
void SixDoFFighter::validate(){
    ...
    // 一旦NED基準での姿勢を計算する
    Quaternion q1=Quaternion::fromAngle(Eigen::Vector3d(0,0,1),motion.getAZ());
...
Eigen::VectorXd SixDoFFighter::trim6(double alt,double V,double tgtBank){
    ...
    std::function<double(unsigned int,const double*,double*,void*)> obj=[&](unsigned int n,const double* x,double* grad,void* data){
        ...
        Eigen::Vector3d gravityVector=motion.dirHtoP(Eigen::Vector3d(0,0,1),Eigen::Vector3d(0,0,0),"NED",false)*m*gravity; //TODO 慣性力モデルに置換
        Eigen::Vector3d Fb=Fab+Ftb+q.conjugate().transformVector(gravityVector);
...
Eigen::VectorXd SixDoFFighter::calcDerivative(const Eigen::VectorXd& x,const Time& t,const Eigen::VectorXd& u){
    //x,uの読み込み
    ...
    //大気,動圧に関する値
    double alt=tmp_motion.getHeight();
    std::map<std::string,double> atm=atmosphere(alt);
    double Mach=V/atm["a"];
    double dp=atm["rho"]*V*V/2;
    double K=dp*S;
    ...
    //状態方程式
    Eigen::Vector3d gravityVector=tmp_motion.dirHtoP(Eigen::Vector3d(0,0,1),Eigen::Vector3d(0,0,0),"NED",false)*m*gravity; //TODO 慣性力モデルに置換
    Eigen::Vector3d Fi=(tmp_motion.relBtoP(CFa)-aero["Cdw"]*vni)*K+tmp_motion.relBtoP(Eigen::Vector3d(thrust,0,0))+gravityVector;
...
void SixDoFFighter::calcMotion(double dt){
    ...
    std::function<Eigen::VectorXd (const Eigen::VectorXd& x,const Time& t)> deriv=[&](const Eigen::VectorXd& x,const Time& t){
        return calcDerivative(x,t,u);
    };
    Eigen::VectorXd nextX=RK4(x0,manager->getTime(),dt,deriv);
    motion.setPos(nextX.block(0,0,3,1));
    motion.setVel(nextX.block(3,0,3,1));
    motion.setOmega(nextX.block(10,0,3,1));
    motion.setQ(Quaternion(nextX(6),nextX(7),nextX(8),nextX(9)).normalized());
...
void SixDoFFlightController::initialize(){
    BaseType::initialize();
    mode="fromDirAndVel";
...
void SixDoFFlightController::serializeInternalState(asrc::core::util::AvailableArchiveTypes & archive, bool full){
    BaseType::serializeInternalState(archive,full);
...
nl::json SixDoFFlightController::getDefaultCommand(){
    return {
        {"ue",0.0},//-1〜1
        {"ua",0.0},//-1〜1
        {"ur",0.0},//-1〜1
...
nl::json SixDoFFlightController::calc(const nl::json &cmd){
...
nl::json SixDoFFlightController::calcDirect(const nl::json &cmd){
    //操舵量を直接指定する場合
    auto p=getShared<SixDoFFighter>(parent);
    double alt=p->motion.getHeight();
...
nl::json SixDoFFlightController::calcFromManualInput(const nl::json &cmd){
...
nl::json SixDoFFlightController::calcFromDirAndVel(const nl::json &cmd){
	...
	if(cmd.contains("dstDir")){
		...
		dstDir.normalize();
        Eigen::Vector3d omegaB=p->motion.relPtoB(p->motion.omega(),p->motion.pos());
        Eigen::Vector3d ex=p->motion.dirBtoP(Eigen::Vector3d(1,0,0));
        Eigen::Vector3d ey=p->motion.dirBtoP(Eigen::Vector3d(0,1,0));
		Eigen::Vector3d down=p->motion.dirHtoP(Eigen::Vector3d(0,0,1),Eigen::Vector3d(0,0,0),"NED",false);
		Eigen::Vector3d eyh=down.cross(ex).normalized();
        Eigen::Vector3d dstDirH=p->motion.dirPtoH(dstDir,p->motion.pos(),"NED",false);
        double dstAz=atan2(dstDirH(1),dstDirH(0));
        Eigen::Vector3d velH=p->motion.velPtoH(p->motion.vel(),p->motion.pos(),"NED",false);
        double vAz=atan2(velH(1),velH(0));
        double vEl=asin(velH(2)/V);
        double dAz=dstAz-vAz;
        while(dAz>M_PI){dAz-=2*M_PI;}
        while(dAz<=-M_PI){dAz+=2*M_PI;}
        Eigen::Vector3d vb=p->motion.relPtoB(p->motion.vel(),p->motion.pos());
        Eigen::Vector3d db=p->motion.relPtoB(dstDir,p->motion.pos());
        if(abs(dAz)>deg2rad(90.0) && vEl>=deg2rad(0.0) && alt<6000){
            double mAz = dAz>0 ? deg2rad(90.0) : -deg2rad(90.0);
            double dxy=sqrt(dstDirH(0)*dstDirH(0)+dstDirH(1)*dstDirH(1));
            Eigen::Vector3d modified(cos(vAz+mAz)*dxy,sin(vAz+mAz)*dxy,dstDirH(2));
            db=p->motion.relHtoB(modified,Eigen::Vector3d(0,0,0),"NED",false);
            ...
            double dxy=sqrt(dstDirH(0)*dstDirH(0)+dstDirH(1)*dstDirH(1));
            Eigen::Vector3d modified(cos(vAz)*dxy,sin(vAz)*dxy,dstDirH(2));
            db=p->motion.relHtoB(modified,Eigen::Vector3d(0,0,0),"NED",false);
        }
...
Eigen::VectorXd SixDoFFlightController::calcU(const nl::json &cmd,const Eigen::VectorXd& x){
    ...
    //大気,動圧に関する値
    double alt=motion.getHeight();
    Eigen::Vector3d dHdr=motion.dirHtoP(Eigen::Vector3d(0,0,1),Eigen::Vector3d(0,0,0),"ENU",false);
    std::map<std::string,double> atm=atmosphere(alt);
    double Mach=V/atm["a"];
    Eigen::Matrix<double,1,17> dMdx=Eigen::Matrix<double,1,17>::Zero();
    dMdx.block(0,0,1,3)=dHdr.transpose()*(-Mach/atm["a"]*atm["dadH"]);
    dMdx.block(0,3,1,3)=vni.transpose()/atm["a"];
    double dp=atm["rho"]*V*V/2;
    double K=atm["rho"]*V*V*p->S*0.5;
    Eigen::Matrix<double,1,17> dKdx=Eigen::MatrixXd::Zero(1,17);
    dKdx.block(0,0,1,3)=dHdr.transpose()*(atm["dRhodH"]*V*V*p->S*0.5);
    ...
    dTdx.block(0,0,1,3)+=dHdr.transpose()*(dTdH);
    dTdx(16)+=dTdP;
    dFidx+=ex*dTdx;
    ...
    // 重力ベクトルの位置による微分は微小なので無視する。
    Eigen::Vector3d gravityVector=motion.dirHtoP(Eigen::Vector3d(0,0,1),Eigen::Vector3d(0,0,0),"NED",false)*p->m*gravity; //TODO 慣性力モデルに置換。
    dFbdx.block(0,6,1,4)+=gravityVector.transpose()*dexdq.block(0,0,3,4);
    dFbdx.block(1,6,1,4)+=gravityVector.transpose()*deydq.block(0,0,3,4);
    dFbdx.block(2,6,1,4)+=gravityVector.transpose()*dezdq.block(0,0,3,4);

    //状態方程式
    Eigen::Vector3d Fab=(CFa-aeroV["Cdw"]*vb/V)*K;
    Eigen::Vector3d Fi=(Rbi*CFa-aeroV["Cdw"]*vni)*K+thrust*ex+gravityVector;
    Eigen::Vector3d Nb=bcbMat*CNa*K;
    ...
    Eigen::Vector3d Fb=Fab+Eigen::Vector3d(thrust,0,0)+q.conjugate().transformVector(gravityVector);
    ...
    Eigen::MatrixXd P=ArimotoPotter(A,B,Q.asDiagonal().toDenseMatrix(),R.asDiagonal().toDenseMatrix());
...
void StevensFighter::initialize(){
    BaseType::initialize();
...
void StevensFighter::serializeInternalState(asrc::core::util::AvailableArchiveTypes & archive, bool full){
    BaseType::serializeInternalState(archive,full);
...
void MorelliFighter::initialize(){
    BaseType::initialize();
...
void MorelliFighter::serializeInternalState(asrc::core::util::AvailableArchiveTypes & archive, bool full){
    BaseType::serializeInternalState(archive,full);
...
void exportSixDoFFighter(py::module &m, const std::shared_ptr<asrc::core::FactoryHelper>& factoryHelper)
{
    ...
    expose_entity_subclass<SixDoFFighter>(m,"SixDoFFighter")
    ...
    ;
    //FACTORY_ADD_CLASS(PhysicalAsset,SixDoFFighter) //Do not register to Factory because SixDoFFighter is abstract class.

    expose_entity_subclass<SixDoFFlightController>(m,"SixDoFFlightController")
    DEF_FUNC(SixDoFFlightController,calcDirect)
    DEF_FUNC(SixDoFFlightController,calcFromManualInput)
    DEF_FUNC(SixDoFFlightController,calcFromDirAndVel)
    DEF_FUNC(SixDoFFlightController,calcU)
    DEF_READWRITE(SixDoFFlightController,PositiveNzLimit)
    DEF_READWRITE(SixDoFFlightController,NegativeNzLimit)
    DEF_READWRITE(SixDoFFlightController,maxNyrCmd)
    DEF_READWRITE(SixDoFFlightController,maxPsCmd)
    DEF_READWRITE(SixDoFFlightController,kPsCmdP)
    DEF_READWRITE(SixDoFFlightController,kPsCmdD)
    DEF_READWRITE(SixDoFFlightController,kNzCmdP)
    DEF_READWRITE(SixDoFFlightController,kNzCmdD)
    DEF_READWRITE(SixDoFFlightController,kNyrCmdP)
    DEF_READWRITE(SixDoFFlightController,kNyrCmdD)
    DEF_READWRITE(SixDoFFlightController,altitudeKeeper)
    ;
    FACTORY_ADD_CLASS(Controller,SixDoFFlightController)

    expose_entity_subclass<MorelliFighter>(m,"MorelliFighter")
    DEF_READWRITE(MorelliFighter,aeroC)
    ;
    FACTORY_ADD_CLASS(PhysicalAsset,MorelliFighter)

    expose_entity_subclass<StevensFighter>(m,"StevensFighter")
