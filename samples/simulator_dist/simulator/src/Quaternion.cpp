...
void Quaternion::fromArray(const Eigen::Vector4d& array){
    q.w()=array(0);
    q.vec()=array.block(1,0,3,1);
}
...
void exportQuaternion(py::module &m)
{
    ...
    expose_common_class<Quaternion>(m,"Quaternion")
        .def(py_init<const double&,const double&,const double&,const double&>())
        .def(py_init<const double&,const Eigen::Vector3d &>())
        .def(py_init<const Eigen::Vector4d &>())
        ...
        DEF_FUNC(Quaternion,fromRotationMatrix)
    ;
}
