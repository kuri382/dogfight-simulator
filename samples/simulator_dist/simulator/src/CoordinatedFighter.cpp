...
void SimpleFighterJetEngine::initialize(){
    BaseType::initialize();
...
void SimpleFighterJetEngine::serializeInternalState(asrc::core::util::AvailableArchiveTypes & archive, bool full){
    BaseType::serializeInternalState(archive,full);
    
    if(full){
        ASRC_SERIALIZE_NVP(archive
            ,enableInstantResponse
            ,considerFuel
            ,tHalf
            ,tminTable
            ,tmilTable
...
void CoordinatedFighter::initialize(){
    BaseType::initialize();
...
void CoordinatedFighter::makeChildren(){
    ...
    if(engine.expired()){
        throw std::runtime_error("CoordinatedFighter requires an engine as its component.");
    }
}

void CoordinatedFighter::serializeInternalState(asrc::core::util::AvailableArchiveTypes & archive, bool full){
    BaseType::serializeInternalState(archive,full);
...
double CoordinatedFighter::calcOptimumCruiseFuelFlowRatePerDistance(){
    ...
    if(ffr<=0.0){
        ...
        std::function<double(unsigned int,const double*,double*,void*)> obj=[&](unsigned int n,const double* x,double* grad,void* data){
            ...
            double tgtCz=-relPtoB(gravityVector)(2)/(dp*S);
...
void CoordinatedFighter::calcMotion(double dt){
    ...
    if(abs(dRoll)>1e-8){
        ...
        motion.setQ(dq1*motion.q);
    }else{
        Eigen::VectorXd dq1=motion.q.dqdwi()*rollAx*dRoll;
        motion.setQ((motion.q+Quaternion(dq1)).normalized());
    }
    //position and velocity update
    Eigen::Vector3d CFb=calcAero(aoa,Mach);
    Eigen::Vector3d F=relBtoP(Eigen::Vector3d(thrust*cos(aoa),0.,-thrust*sin(aoa))+CFb*(dp*S))+gravityVector;
    ...
    if(rotAngle<1e-8){
        ...
        motion.setQ((motion.q+Quaternion(dq2)).normalized());
    }else{
        ...
        motion.setQ(dq2*motion.q);
    }
    Quaternion dq=motion.q*qPrev.conjugate();
    Eigen::Vector3d ax=motion.q.vec();
    double sn=ax.norm();
    if(sn>0){ax/=sn;}
    motion.setOmega(ax*atan2(sn,dq.w())+rollAx*dRoll/dt);
}

void CoordinatedFlightController::initialize(){
    BaseType::initialize();
    mode="fromDirAndVel";
...
void CoordinatedFlightController::serializeInternalState(asrc::core::util::AvailableArchiveTypes & archive, bool full){
    BaseType::serializeInternalState(archive,full);
...
nl::json CoordinatedFlightController::getDefaultCommand(){
    return {
        {"roll",0.0},
        {"pitch",0.0},
        {"accel",0.0}
...
nl::json CoordinatedFlightController::calc(const nl::json &cmd){
...
nl::json CoordinatedFlightController::calcDirect(const nl::json &cmd){
    ...
    Eigen::Vector3d gravityVector = down*p->m*gravity; //TODO 慣性力モデルに置換。
    double tgtCz=-p->relPtoB(gravityVector)(2)/(dp*p->S);
    ...
    //side G Limitter
    Eigen::Vector3d ey=p->relBtoP(Eigen::Vector3d(0,cos(dRoll_),sin(dRoll_)));
    Eigen::Vector3d ez=p->relBtoP(Eigen::Vector3d(0,-sin(dRoll_),cos(dRoll_)));
    double zz=p->calcAero(aoa_,Mach)(2)/(p->m*gravity)+ez.dot(down);
    if(Eigen::Vector3d(0,ey.dot(down),zz).norm() >= p->sideGLimit){
        if(zz>=0){
            tgtCz=p->m*util::gravity*(sqrt(p->sideGLimit-ey.dot(down)*ey.dot(down))-ez.dot(down));
        }else{
            tgtCz=-p->m*util::gravity*(sqrt(p->sideGLimit-ey.dot(down)*ey.dot(down))-ez.dot(down));
        }
...
nl::json CoordinatedFlightController::calcFromDirAndVel(const nl::json &cmd){
    auto p=getShared<CoordinatedFighter>(parent);
    double eps=1e-4;
    double alt=p->motion.getHeight();
    double V=p->motion.vel.norm();
    ...
    double Mach=V/atm["a"];
    double dp=atm["rho"]*V*V/2;
    Eigen::Vector3d gravityVector = p->motion.dirHtoP(Eigen::Vector3d(0,0,1),Eigen::Vector3d(0,0,0),"NED",false)*p->m*gravity; //TODO 慣性力モデルに置換。
    Eigen::Vector3d gravityVectorVert=gravityVector-(gravityVector.dot(d))*d;//==g.dot(n)+g.dot(b) for any φ
    Eigen::Vector3d b,n;
    auto eng=getShared<SimpleFighterJetEngine>(p->engine);
	...
	if(cmd.contains("dstDir")){
		...
		if(snTheta<eps){
	    	...
	    	b=gravityVectorVert.normalized();
    ...
    n=b.cross(d);
    double gt=gravityVector.dot(d)/p->m;
    double gb_v=gravityVector.dot(b)/(p->m*V);
    double gn_v=gravityVector.dot(n)/(p->m*V);
	...
	Eigen::Vector3d eyPrev=p->relBtoP(Eigen::Vector3d(0.,1.,0.));
...
void exportCoordinatedFighter(py::module &m, const std::shared_ptr<asrc::core::FactoryHelper>& factoryHelper)
{
    using namespace pybind11::literals;
    expose_entity_subclass<SimpleFighterJetEngine>(m,"SimpleFighterJetEngine")
    ...
    expose_entity_subclass<CoordinatedFighter>(m,"CoordinatedFighter")
    ...
    expose_entity_subclass<CoordinatedFlightController>(m,"CoordinatedFlightController")
    DEF_FUNC(CoordinatedFlightController,calcDirect)
    DEF_FUNC(CoordinatedFlightController,calcFromDirAndVel)
    DEF_READWRITE(CoordinatedFlightController,lambdaVel)
    DEF_READWRITE(CoordinatedFlightController,lambdaTheta)
    DEF_READWRITE(CoordinatedFlightController,clampTheta)
    DEF_READWRITE(CoordinatedFlightController,kVel)
    DEF_READWRITE(CoordinatedFlightController,kAccel)
    DEF_READWRITE(CoordinatedFlightController,kPower)
    DEF_READWRITE(CoordinatedFlightController,kTheta)
    DEF_READWRITE(CoordinatedFlightController,kOmega)
    DEF_READWRITE(CoordinatedFlightController,kAoa)
    DEF_READWRITE(CoordinatedFlightController,kEps)
    DEF_READWRITE(CoordinatedFlightController,pitchLimit)
    DEF_READWRITE(CoordinatedFlightController,pitchLimitThreshold)
    DEF_READWRITE(CoordinatedFlightController,altitudeKeeper)
    ;
    FACTORY_ADD_CLASS(Controller,CoordinatedFlightController)
}
