...
PYBIND11_NAMESPACE_BEGIN(fallback)
    inline std::false_type is_default_type_caster_disabled_impl(...);
PYBIND11_NAMESPACE_END(fallback)
PYBIND11_NAMESPACE_BEGIN(specialized)
    template<typename type>
    struct tag {};
PYBIND11_NAMESPACE_END(specialized)
template<typename type>
constexpr bool is_default_type_caster_disabled(){
    using fallback::is_default_type_caster_disabled_impl;
    return decltype(is_default_type_caster_disabled_impl(std::declval<specialized::tag<type>>()))::value;
}

#define PYBIND11_DISABLE_DEFAULT_TYPE_CASTER(...)                                                 \
    PYBIND11_NAMESPACE_BEGIN(PYBIND11_NAMESPACE)                                                  \
    namespace detail {                                                                            \
    namespace specialized {                                                                       \
    inline std::true_type is_default_type_caster_disabled_impl(tag<__VA_ARGS__>);   \
    }                                                                                             \
    }                                                                                             \
    PYBIND11_NAMESPACE_END(PYBIND11_NAMESPACE)

#define PYBIND11_ENABLE_IF_DEFAULT_TYPE_CASTER_DISABLED(...)                                      \
    typename std::enable_if<                                                                      \
        ::pybind11::detail::is_default_type_caster_disabled<__VA_ARGS__>(),void>::type

#define PYBIND11_ENABLE_IF_DEFAULT_TYPE_CASTER_ENABLED(...)                                       \
    typename std::enable_if<                                                                      \
        !::pybind11::detail::is_default_type_caster_disabled<__VA_ARGS__>(),void>::type
...
class type_caster<std::pair<T1, T2>, PYBIND11_ENABLE_IF_DEFAULT_TYPE_CASTER_ENABLED(std::pair<T1, T2>)>
    : public tuple_caster<std::pair, T1, T2> {};

template <typename... Ts>
class type_caster<std::tuple<Ts...>, PYBIND11_ENABLE_IF_DEFAULT_TYPE_CASTER_ENABLED(std::tuple<Ts...>)>
    : public tuple_caster<std::tuple, Ts...> {};

template <>
class type_caster<std::tuple<>, PYBIND11_ENABLE_IF_DEFAULT_TYPE_CASTER_ENABLED(std::tuple<>)>
    : public tuple_caster<std::tuple> {
public:
...
#define PYBIND11_MAKE_OPAQUE(...)                                                                 \
    PYBIND11_DISABLE_DEFAULT_TYPE_CASTER_FOR_STL_CONTAINER(__VA_ARGS__)                           \
    PYBIND11_NAMESPACE_BEGIN(PYBIND11_NAMESPACE)                                                  \
    namespace detail {                                                                            \
    template <>                                                                                   \
