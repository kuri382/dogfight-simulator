...
from ASRCAISim1.plugins.torch_truncnorm import TruncatedNormal
...
class ActionDistributionBase:
    ...
    """
    def __init__(self, model, params, space=None, legal_actions=None, validate_args=None):
        # ===== for backward compatibility =====
        # params, model
        if isinstance(params, nn.Module):
            import warnings
            warnings.warn("Mandatory constructor arguments of HandyRLUtility.ActionDistribution has been changed from (params, model) to (model, params, space). This backward compatibility will be removed in the future.", DeprecationWarning)
            tmp=model
            model=params
            params=tmp
    ...
    def getInvalidActionMask(cls,space):
        return np.full([cls.get_param_dim(space)], torch.inf)
...
class DiscreteActionDistribution(ActionDistributionBase):
    ...
    """
    def __init__(self, model, params, space=None, legal_actions=None, validate_args=None):
        super().__init__(model, params, space, legal_actions, validate_args)
        assert(isinstance(self.space,gym.spaces.Discrete))
        self.ndim=self.space.n
        self.action_mask=None
        self.exploration_logits=None
        if(self.asNumpy):
            self.exploration_logits=np.zeros_like(self.params)
        else:
            self.exploration_logits=torch.zeros_like(self.params)
        if(legal_actions is not None):
            if(self.asNumpy):
                self.action_mask = np.ones_like(self.params) * np.inf
            else:
                self.action_mask = torch.ones_like(self.params) * torch.inf
            self.action_mask[legal_actions==1] = 0
            self.params = self.params - self.action_mask
        ...
        self.exploration_logits=torch.where(self.params==-torch.inf,-torch.inf,self.exploration_logits)
        self.dist=dist.Categorical(logits=self.params, validate_args=validate_args)
    def sample(self,deterministic=False,exploration=None):
...
class MultiDiscreteActionDistribution(ActionDistributionBase):
    ...
    def __init__(self, model, params, space=None, legal_actions=None, validate_args=None):
        ...
        self.action_mask=None
        splitted_params=self.unpack_policy(self.params)
        self.exploration_logits=None
        if(self.asNumpy):
        ...
        if(legal_actions is not None):
            ...
            for i in range(len(splitted_params)):
                p=splitted_params[i]
                if(self.asNumpy):
                    mask = np.ones_like(p) * np.inf
                else:
                    mask = torch.ones_like(p) * torch.inf
        ...
        self.exploration_logits=[torch.where(p==-torch.inf,-torch.inf,el) for p,el in zip(splitted_params,self.exploration_logits)]
...
class BoxActionDistribution(ActionDistributionBase):
    ...
    def __init__(self, model, params, space=None, legal_actions=None, validate_args=None):
        ...
        self.action_mask=None
        mean_, stddev_ = self.unpack_policy(self.params)
        if(self.asNumpy):
            mean_ = torch.tensor(mean_)
            stddev_ = torch.tensor(stddev_)
            self.action_mask = np.zeros_like(self.params)
        else:
            self.action_mask = torch.zeros_like(self.params)
        if(legal_actions is not None):
...
class TupleActionDistribution(ActionDistributionBase):
    ...
    def __init__(self, model, params, space=None, legal_actions=None, validate_args=None):
        ...
        if(self.asNumpy):
            splitted_params = np.split(self.params, [np.sum(dims[:i+1]) for i in range(len(dims)-1)], -1)
        else:
            splitted_params = torch.split(self.params, dims, -1)
        legal_actions = legal_actions if legal_actions is not None else [None for sub in self.space.spaces]
        self.dists = [cl(self.model, p, sub, l, validate_args=validate_args)
...
class DictActionDistribution(ActionDistributionBase):
    ...
    def __init__(self, model, params, space=None, legal_actions=None, validate_args=None):
        ...
        self.dists={k: self.dist_classes[k](self.model, splitted_params[k], sub, legal_actions[k], validate_args=validate_args)
            for k, sub in self.space.spaces.items()
        }
