...
try:
    from .R7ContestGodView import R7ContestGodView
    factoryHelper.addPythonClass('Viewer','R7ContestGodView',R7ContestGodView)
except Exception as ex:
    ex_str=str(ex)
    from ASRCAISim1.core import Viewer
    class R7ContestGodView(Viewer):
        def __init__(self,modelConfig,instanceConfig):
            super().__init__(modelConfig,instanceConfig)
            self.exception=ex_str
            print("============================")
            print("Warining: R7ContestGodView was not imported collectly. The exception occured is,")
            print(self.exception)
            print("To continue simulation, a dummy no-op Viewer is used instead.")
            print("============================")
    factoryHelper.addPythonClass('Viewer','R7ContestGodView',DummyR7ContestGodView)
